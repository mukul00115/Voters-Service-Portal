<?php
	...
	while($row = $query->fetch_assoc()){
		...
		while($crow = $cquery->fetch_assoc()){
			...
			$candidate .= '
				<li>
					'.$input.'<button class="btn btn-primary btn-sm btn-curve clist platform-btn" data-id="'.$crow['id'].'" style="background-color: #4682B4; color: black; font-size: 12px; font-family: Times"><i class="fa fa-search"></i> Platform</button><img src="'.$image.'" height="100px" width="100px" class="clist"><span class="cname clist">'.$crow['firstname'].' '.$crow['lastname'].'</span>
				</li>
			';
		}
		...
		$output .= '
			<div class="row">
				<div class="col-xs-12">
					<div class="box box-solid" style="background-color: #e0e0e0; border: 1px solid #ccc;" id="'.$row['id'].'">
